...
export const ReporterDashboard = () => {
  ...
  const fetchMyStories = async () => {
    try {
      ...
      // Filter by current user or show user's reported stories
      if (currentUser) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-200">
      ...
      <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 flex flex-wrap items-center justify-between gap-6 bg-gradient-to-br from-slate-900 via-slate-950 to-emerald-950/40">
        <div className="flex items-center gap-4">
          {currentUser?.avatar ? (
            <img src={currentUser.avatar} alt="Avatar" className="w-16 h-16 rounded-2xl object-cover ring-2 ring-emerald-500/40 shadow-xl" />
          ) : (
          ...
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-bold text-white font-serif">{currentUser?.name || 'Ananya Sharma'}</h1>
              <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-mono font-bold border border-emerald-500/30">
                Verified Citizen Journalist
              </span>
            </div>
            <p className="text-xs text-slate-300 max-w-xl">{currentUser?.bio || 'Hyperlocal civic reporter documenting urban infrastructure & water logging.'}</p>
            <div className="text-[11px] text-slate-400 flex items-center gap-3 pt-1">
              <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-rose-400" /> {currentUser?.location || 'Mumbai, MH'}</span>
              <span>•</span>
              <span className="flex items-center gap-1 text-emerald-400 font-medium"><Award className="w-3.5 h-3.5" /> {currentUser?.reputationScore || 94} Reputation Points</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-center">
        <div className="glass-card p-4 rounded-2xl border border-slate-800">
          <div className="text-2xl font-extrabold text-white font-mono">{myStories.length}</div>
          <div className="text-xs text-slate-400 mt-1">Total Submissions</div>
        </div>

        <div className="glass-card p-4 rounded-2xl border border-slate-800">
          <div className="text-2xl font-extrabold text-emerald-400 font-mono">{publishedCount}</div>
        ...
        <div className="glass-card p-4 rounded-2xl border border-slate-800">
          ...
          <div className="text-xs text-slate-400 mt-1">Under Fact-Check</div>
        </div>

        <div className="glass-card p-4 rounded-2xl border border-slate-800">
          <div className="text-2xl font-extrabold text-blue-400 font-mono">{editsCount}</div>
          <div className="text-xs text-slate-400 mt-1">Edits Requested</div>
        </div>
      </div>

      {/* My Submitted Stories List */}
      <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <h2 className="text-base font-bold text-white flex items-center gap-2">
            <FileText className="w-5 h-5 text-emerald-400" />
            <span>My Citizen Reporting History</span>
          </h2>
              ...
              >
                ...
                <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-2">
                  <div className="flex items-center justify-between text-[11px] font-mono">
                    <span className="text-slate-400">Verification Lifecycle Status:</span>
                  ...
                  {story.reviewerNotes && (
                    <div className="text-xs text-slate-300 italic bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                      <strong className="text-amber-400 font-sans not-italic">Fact-Checker Feedback: </strong>
                      {story.reviewerNotes}
                    </div>
                  )}
                </div>
                ...
                <div className="pt-2 flex items-center justify-between text-xs text-slate-400">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5 text-slate-400" /> {story.comments?.length || 0} Comments</span>
                  </div>
